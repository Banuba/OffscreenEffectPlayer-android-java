...
{   
    ...
    float screenRatio_y = glfx_SCREEN.y/glfx_SCREEN.x;
    float screenRatio_x = glfx_SCREEN.x/glfx_SCREEN.y;

    vec4 bg = texture(glfx_BACKGROUND,uv);

    float angle = js_bg_rotation.x;

    uv = rotate_uv(uv,angle);
	uv = scale_uv(uv, js_bg_scale.x);

    if(angle == -90. || angle == 270.){
        uv.x += screenRatio_y - 1.;
        uv.x /= screenRatio_y;
        uv.y *= screenRatio_y;
    } else if (angle == 90.) {
        uv.x /= screenRatio_y;
